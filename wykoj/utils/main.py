...
from typing import Any, Callable, Awaitable, Optional, List, Tuple, Union
...
def contest_redirect(f: Callable[..., Any]) -> Callable[..., Any]:
    ...
    async def inner(*args: Any, **kwargs: Any) -> Any:
        ...
        if (contest and await current_user.is_authenticated and not current_user.is_admin
...
def join_authors(authors: Union[List[User], ManyToManyRelation[User]]) -> str:
    if not authors:
        return ""
    usernames = [f'<a href="{url_for("main.user_page", username=a.username)}">{a.username}</a>' for a in authors if a]
    if len(usernames) == 1:
        return usernames[0]
...
def join_contests(contests: Union[List[Contest], ManyToManyRelation[Contest]]) -> str:
    if not contests:
        return ""
    contests = [f'<a href="{url_for("main.contest_page", contest_id=c.id)}">{c.title}</a>' for c in contests if c]
    if len(contests) == 1:
        return contests[0]
