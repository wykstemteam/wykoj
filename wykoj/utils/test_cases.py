...
async def get_config(task_id: str) -> Optional[Dict[str, Any]]:
    try:
        config = json.loads(
            await
            read_file(os.path.join(current_app.root_path, "test_cases", task_id, "config.json"))
        )
...
async def get_sample_test_cases(task_id: str) -> List[Tuple[str, str]]:
    ...
    try:
        files = set(
            f for f in os.listdir(os.path.join(current_app.root_path, "test_cases", task_id))
    ...
    for i in count(1):
        if f"0.{i}.in" in files and f"0.{i}.out" in files:
            ...
            case_in = await read_file(
                os.path.join(current_app.root_path, "test_cases", task_id, f"0.{i}.in")
            )
            case_out = await read_file(
                os.path.join(current_app.root_path, "test_cases", task_id, f"0.{i}.out")
            )
